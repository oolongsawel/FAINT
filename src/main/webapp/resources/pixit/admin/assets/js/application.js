...
if ((curpage == "" || curpage == "/" || curpage == "admin") && hash == "") {
	//alert(curpage);
	$("nav#sidebar ul li a[href*='" + "dashboard" + "']").addClass("current").addClass("active");
	...
	//alert(hash);
	//alert(curpage);
	$("nav#sidebar ul li").each(function() {
	...
	if (hash != "") {
		$("nav#sidebar ul li a[href*='" + hash + "']").parents("li").addClass("active");
		$("nav#sidebar ul.submenu li a[href*='" + hash + "']").addClass("current").addClass("active");
	} else {
		$("nav#sidebar ul li a[href*='" + curpage + "']").parents("li").addClass("active").addClass("current").addClass("hasSub");
		$("nav#sidebar ul.submenu li a[href*='" + curpage + "']").addClass("current").addClass("active");
	}
	 
}

/*
 * $(document).ready(function () { //current active var url = window.location;
 * alert(url); $('nav#sidebar ul li a[href="'+ url
 * +'"]').parent().addClass('active'); $('nav#sidebar ul li
...
if ($.fn.sparkline) {
	...
	if ($.cookie('style-color') == 'dark') {
		sparkline1_color = '#159077';
		sparkline2_color = '#00699e';
		sparkline3_color = '#9e494e';
	}
	if ($.cookie('style-color') == 'red') {
		sparkline1_color = '#121212';
		sparkline2_color = '#4AB2F8';
		sparkline3_color = '#E0A832';
	}
	if ($.cookie('style-color') == 'blue') {
		sparkline1_color = '#E0A832';
		sparkline2_color = '#D9534F';
		sparkline3_color = '#121212';
	}
	if ($.cookie('style-color') == 'green') {
		sparkline1_color = '#E0A832';
		sparkline2_color = '#D9534F';
		sparkline3_color = '#121212';
	}
	...
	/*
	 * Sparklines can also take their values from the first argument passed to
	 * the sparkline() function
	 */
...
function chatSidebar() {
	...
	$('.send-message').keypress(function(e) {
		if (e.keyCode == 13) {
			var chat_message = '<li class="img">' + '<span>' + '<div class="chat-detail chat-right">' + '<img src="assets/img/avatars/avatar1.png" data-retina-src="assets/img/avatars/avatar1_2x.png"/>' + '<div class="chat-detail">' + '<div class="chat-bubble">' + $(this).val() + '</div>' + '</div>' + '</div>' + '</span>' + '</li>';
...
function liveTile() {
	$('.live-tile').each(function() {
		...
		$(this).liveTile({
			speed : $(this).data("speed") ? $(this).data("speed") : 500, // Start
																			// after
																			// load
																			// or
																			// not
			mode : $(this).data("animation-easing") ? $(this).data("animation-easing") : 'carousel', // Animation
																										// type:
																										// carousel,
																										// slide,
																										// fade,
																										// flip,
																										// none
			playOnHover : $(this).data("play-hover") ? $(this).data("play-hover") : false, // Play
																							// live
																							// tile
																							// on
																							// hover
			repeatCount : $(this).data("repeat-count") ? $(this).data("repeat-count") : -1, // Repeat
																							// or
																							// not
																							// (-1
																							// is
																							// infinite
			delay : $(this).data("delay") ? $(this).data("delay") : 0, // Time
																		// between
																		// two
																		// animations
			startNow : $(this).data("start-now") ? $(this).data("start-now") : true, // Start
																						// after
																						// load
																						// or
																						// not
...
if ($('.animate-number').length && $.fn.numerator) {
	$('.animate-number').each(function() {
		$(this).numerator({
			easing : $(this).data("animation-easing") ? $(this).data("animation-easing") : 'linear', // easing
																										// options.
			duration : $(this).data("animation-duration") ? $(this).data("animation-duration") : 700, // the
																										// length
																										// of
																										// the
																										// animation.
...
if ($('.sparkline').length && $.fn.sparkline) {
	$('.sparkline').each(function() {
		$(this).sparkline($(this).data("sparkline-value"), {
...
if ($('.summernote').length && $.fn.summernote) {
	$('.summernote').each(function() {
		$(this).summernote({
			height : 600,
			toolbar : [ [ "style", [ "style" ] ], [ "style", [ "bold", "italic", "underline", "clear" ] ], [ "fontsize", [ "fontsize" ] ], [ "color", [ "color" ] ], [ "para", [ "ul", "ol", "paragraph" ] ], [ "height", [ "height" ] ], [ "table", [ "table" ] ] ]
		});
	});
}
