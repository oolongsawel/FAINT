...
public class UserServiceImpl implements UserService {
	...
	//회원 정보 변경
	@Override
	public UserVO modifyUser(UserVO vo) throws Exception {
		System.out.println("모디파이1");
		dao.updateUser(vo);
		System.out.println("모디파이 2");
		return  dao.userRead(vo);
	}
...
//  }
	...
	public void findPassword(UserVO user) throws Exception {
		...
		dao.updateAuthKey(user.getEmail(),key); //인증키 db 저장

		MailHandler sendMail = new MailHandler(mailSender);
		sendMail.setSubject("[서어비스 센터 비밀번호 찾기 ]");
		sendMail.setText(
		new StringBuffer().append("<h1>아래 주소로 접속하여 패스워드를 변경해주세요</h1>").append("<a href='http://localhost:8080/user/findPasswordConfirm?userEmail=").append(user.getEmail()).append("&memberAuthKey=").append(key).append("' target='_blank'>이메일 인증 확인</a>").toString());
