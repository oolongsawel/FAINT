...
public class MemberController {
    ...
    public void registerGET(MemberVO board, Model model) throws Exception {
	...
	public @ResponseBody
	String checkDuplicate(HttpServletResponse response, @RequestParam("memberEmail") String userEmail, Model model)throws Exception {
    ...
    public @ResponseBody
    String checkDuplicateName(HttpServletResponse response, @RequestParam("memberName") String memberName, Model model)throws Exception {
        System.out.println("/authenticateName 진입");
        
        String msg = service.authenticateName(memberName);
	...
	public String emailConfirm(MemberVO user,Model model,RedirectAttributes rttr) throws Exception { // 이메일인증
    ...
    public void findPasswordGET(MemberVO user, Model model) throws Exception {
        //System.out.println("password 찾기 GET 진입");

    }
	//이메일 확인 후
    @RequestMapping(value = "/findPassword", method = RequestMethod.POST)
    public String findPasswordPost(MemberVO user,Model model,RedirectAttributes rttr) throws Exception{
        //System.out.println("find passwordPost 진입 ");
		//userEmail 값으로 회원 여부 확인 requestparam 으로 변경??
        String str= user.getMemberEmail();
    ...
    public String passwordSetConfirm(MemberVO user, Model model, RedirectAttributes rttr) throws Exception { // 이메일인증
       // System.out.println("컨트롤러 입력 정보: "+user);
    	MemberVO vo=service.userAuthFindPassword(user);
        //System.out.println("controller 서비스에서 받은: "+vo);
		if(vo == null) {
			rttr.addFlashAttribute("msg" , "비정상적인 접근 입니다. 다시 인증해 주세요");
			return "redirect:/";
		}
		int id=vo.getMemberID();
    ...
    public void setPassUserAuthGet(MemberVO user,Model model) throws Exception{
	...
	public void setPassword(MemberVO user, Model model, RedirectAttributes rttr) throws Exception {
		System.out.println("setPassword GET");

	}
	//패스워드 변경
	@RequestMapping(value = "/setPassword", method = RequestMethod.POST)
	public String setPasswordPost(MemberVO user, Model model, RedirectAttributes rttr) throws Exception {
	...
	public void ModifyUserAuthGet(MemberVO user,Model model) throws Exception{
	...
	public String ModifyUserPost(MemberVO user,Model model,RedirectAttributes rttr,HttpSession session, MultipartFile file) throws Exception{
		//profile 정보 확인
        String test=user.getMemberPicture();
        //user 정보 저장
		String userId= user.getMemberID()+"";
		MemberVO ssvo = (MemberVO)session.getAttribute("login");
		...
		if(!file.isEmpty()) {
           ...
           System.out.println("파일 업로드 완료");
            user.setMemberPicture(uploadedFileName);
        }else{
			//System.out.println("imagedefualt: "+imagedefualt);
			if(imagedefualt.equals("basic")){
				user.setMemberPicture(null);
				//System.out.println("basic"+user.getUserProfile());
			}else {

				//프로필 업로드 하지 않을 때 원래 정보를 저장
				MemberVO vot = (MemberVO) session.getAttribute("login");
				user.setMemberPicture(vot.getMemberPicture());
				//System.out.println("no basic"+user.getUserProfile());
			}
		    }


		MemberVO vo=service.modifyUser(user);
